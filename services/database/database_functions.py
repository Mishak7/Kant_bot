...
async def add_tasks(
            ...
            if not row:
                ...
                if answer is not None:
                    await cursor.execute("""
                        ...
                        VALUES (?, ?)
                    """, (task_id, answer))
                logger.info(f"Добавлено новое задание: {task_question}")
            else:
                task_id = row[0]
                if update_existing:
                    ...
                    if answer is not None:
                        ...
                        if existing_answer:
                            await cursor.execute("""
                                                            UPDATE TasksAnswers
                                                            SET correct_answer=? WHERE task_id=?
                            """, (answer, task_id))
    ...
    async with aiosqlite.connect('BFU.db') as db:
        if data is not None:
            ...
            logger.info(f"Начало обработки {len(tasks)} заданий")
            for i, task in enumerate(tasks, 1):
                logger.info(f"Обработка задания {i}/{len(tasks)}: {task.get('task_question', 'No question')}")
            ...
            logger.info("Все задания обработаны")
        else:
            await add_single_task(
                db,
...
async def all_tasks_done_right(user_id, level):
    ...
    try:
        async with aiosqlite.connect('BFU.db') as db:
            ...
            cursor = await db.execute("""
                SELECT COUNT(DISTINCT up.task_id) 
                FROM UserProgress up
                JOIN Tasks t ON up.task_id = t.task_id
                WHERE up.user_id = ? AND up.is_correct = ? AND t.level_id = ?
            """, (user_id, True, level_id))
    ...
    try:
        async with aiosqlite.connect('BFU.db') as db:
            ...
            cursor = await db.execute("""
                ...
                FROM UserProgress up
                JOIN Tasks t ON up.task_id = t.task_id
                WHERE up.user_id = ? AND t.level_id = ?
            """, (user_id, level_id))
